...
def evaluate_model(checkpoint_path, activation="elu", gate_type_coarse="regular_conv", gate_type_fine="regular_conv", kernel_size=3):
    # Load the checkpoint
    ...
    pytorch_output = pytorch_output[0].detach().numpy() * 0.5 + 0.5
    ...
    for i in range(10):
        ...
        print("Comparison round", i, "decimals:", np.array_equal(arr1, arr2), np.count_nonzero(np.sum(np.abs(arr1 - arr2), axis=2)))

    pytorch_synthesis = np.zeros_like(pytorch_output)
    pytorch_synthesis[nn_msk[0][0].detach().numpy() > 0] = pytorch_output[nn_msk[0][0].detach().numpy() > 0]
    onnx_synthesis = np.zeros_like(onnx_output)
    onnx_synthesis[nn_msk[0][0].detach().numpy() > 0] = onnx_output[nn_msk[0][0].detach().numpy() > 0]
    print("Mean absolute difference:", np.mean(np.abs(pytorch_synthesis - onnx_synthesis)))
    print("Mean relative difference:", np.mean(np.abs((pytorch_synthesis - onnx_synthesis) / (pytorch_synthesis + epsilon))))
...
evaluate_model(checkpoint_path, activation="elu", gate_type_coarse="regular_conv", gate_type_fine="regular_conv", kernel_size=3)
...
evaluate_model(checkpoint_path, activation="tanh", gate_type_coarse="depth_separable", gate_type_fine="depth_separable", kernel_size=4)
